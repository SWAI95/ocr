...
rr = sub.add_run("테스트셋: 한국어 계약서 doc_00 (5페이지)  ·  지표: CER/WER  ·  평가 기준 GT 대비")
rr.font.size = Pt(10)
...
b1.bold = True
b1.font.size = Pt(12)
b2 = box.add_run("하이브리드(hybrid_vl) 엔진이 3개 문서 모두 CER 최저(평균 ~1.2%)로 종합 1위 — 한국어 문서 OCR 베스트")
b2.font.size = Pt(12)
...
add_table(
    ...
    [
        ...
        ["paddle_vl", "PaddleOCR-VL-0.9B + PP-DocLayoutV3",
         "단독 VLM(서버 불필요). 단 5090(Blackwell) 미지원 → 4090 전용", "△ 4090 배포"],
...
add_table(
    doc,
    ["엔진", "doc_00", "doc_04", "doc_05", "평균 CER"],
    [
        ["hybrid_vl ★", "~1.7 / ~14.6", "0.75 / 4.2", "1.32 / 5.4", "~1.2%"],
        ["paddle", "2.28 / 43.2", "1.15 / 35.8", "2.48 / 32.4", "1.97%"],
        ["ollama_vl", "~2.8 / ~13.6", "1.89 / 4.9", "1.53 / 3.9", "~2.1%"],
        ["easyocr", "11.4 / 45.0", "8.02 / 34.2", "8.89 / 32.9", "9.4%"],
        ["tesseract", "33.1 / 168", "28.7 / 174", "16.4 / 199", "26.0%"],
        ["paddle_vl", "4090 전용 (5090 미측정)", "—", "—", "—"],
    ],
    best_row=0,
    center_cols=(1, 2, 3, 4),
)
para(doc, "doc_00=계약서 5p · doc_04=도급계약서 11p · doc_05=계약서 12p (총 28p). "
          "hybrid_vl이 3개 문서 모두 CER 최저 → 종합 1위. doc_04·05는 doc_00보다 깨끗해 "
          "VLM WER이 4~5%로 매우 낮다.", italic=True, color=GRAY, size=9)
...
para(doc, "※ 측정 환경: 개발기 RTX 5090(Blackwell) + paddlepaddle-gpu cu129(CUDA 12.9). cu129 "
          ...
          "폴백했음. paddle_vl은 5090(Blackwell)에서 VLM 인식이 빈 출력이라 제외(4090 전용), "
          "tesseract는 바이너리 설치 후 측정(CER 33.1%, 가장 약한 baseline).",
     italic=True, color=GRAY, size=9)
...
ENGINES = [
    {
        ...
        "how": "페이지를 먼저 레이아웃으로 분류해, 표·폼이 많으면 Paddle(구조·글자 정확), 줄글이 "
               ...
               "병합해 구제한다.",
        "pros": ["CER ~1.7%(전체 최저) · WER ~14.6%(가독성 상위)로 종합 1위",
    ...
    },
    {
        ...
        "how": "0.9B 경량 VLM을 in-process(native)로 실행 — 별도 추론 서버 불필요. 이미지 → "
               "레이아웃 + 블록별 VLM 인식 → 마크다운/표.",
        "pros": ["단독 VLM으로 레이아웃+OCR(외부 서빙 백엔드 불필요)",
                 "109개 언어 지원, 경량(0.9B)",
                 "배포 4090(sm_89)에서 GPU 네이티브 동작"],
        "cons": ["VLM의 fused 커널(rms_norm·sparse flash-attn)이 sm_120(Blackwell) 미지원 → 5090서 인식 빈 출력",
                 "conv(레이아웃)는 cu129로 동작하나 transformer 생성부가 안 됨 → 4090 GPU 전용",
                 "가중치 사전 다운로드 필요"],
        "fit": "4090 등 sm_89 GPU 배포 환경에서 VLM 단독 인식이 필요할 때.",
    },
    {
...
MODELS = [
    ...
    },
    {
        ...
        "cons": ["한국어 줄글 띄어쓰기 약함(WER↑)", "5090(Blackwell) CPU 동작"],
    ...
    },
    {
        ...
        "pros": ["단독 VLM(외부 서빙 불필요)", "경량·다국어", "4090 GPU 네이티브"],
        "cons": ["5090(Blackwell) 미지원(dev 실행 불가)", "가중치 사전 다운로드 필요"],
        "used": "paddle_vl",
    },
    {
...
bullets(doc, [
    ("종합 1위: ", "doc_00·04·05 3개 문서 모두 CER 최저(평균 ~1.2%) — 정확도 1위 + 가독성 상위(WER)로 종합 최적."),
...
add_table(
    ...
    [
        ...
        ["4090 배포 + VLM 단독", "paddle_vl", "서버 불필요 VLM(4090 GPU)"],
